...
export function calculateHP(character: CharacterInstance): CharacterInstance {
  ...
  for (const prof of returnCharacter.classes) {
    ...
    const hitDie = prof.hitDiceValue / 2 + 1;
    returnCharacter.maxHP += hitDie * prof.classLevel;
  }

  const conMod = Math.trunc(
    (returnCharacter.calculatedStats.constitution - 10) / 2
  );
  returnCharacter.maxHP += conMod * returnCharacter.level;
  returnCharacter.remainingHP = returnCharacter.maxHP;
  return returnCharacter;
}

export function establishItemBonuses(
  ...
  for (const equip of character.items) {
    switch (equip.modifier.affectedObject) {
      case "stats":
        if (
          hasKey(
            returnCharacter.calculatedStats,
            equip.modifier.affectedValue
          ) &&
          typeof equip.modifier.value == "number"
        ) {
          returnCharacter.calculatedStats[equip.modifier.affectedValue] +=
            equip.modifier.value;
        }
        break;
      case "maxHP":
        if (typeof equip.modifier.value == "number")
          returnCharacter.maxHP += equip.modifier.value;
        break;
      case "tempHP":
        if (
          typeof equip.modifier.value == "number" &&
          equip.modifier.value > returnCharacter.tempHP
        )
